...
**Date of research:** 2026-09-22. Evidence correction the same day, after re-retrieval of the Smart View SDK download page [S12] and the Expo SDK reference [S60]. The correction does not select an architecture.
...
**Documented OS envelope for SDK 57.** Expo's SDK reference compatibility table lists SDK 57.0.0 as Android 7+, `compileSdkVersion` 36, `targetSdkVersion` 36, iOS 16.4+, and Xcode 26.4+. [S60] Those figures are factual constraints of an app built on the documented SDK 57 template. They are not a decision that AppT accepts them. The iOS 16.4+ minimum is the one that must be weighed before architecture selection. The table lists it as the minimum that SDK supports. This research did not test whether a lower deployment target still builds. A fully native app, or a different stack, could potentially set a different minimum-OS envelope, subject to store rules that are independent of Expo. This report does not decide whether that minimum is acceptable.
...
**Smart View SDK (official, and a different product).** Samsung's Smart View SDK is a multiscreen SDK. The sender discovers compatible televisions, launches a receiver application, and communicates with that application over a channel. The getting-started page says a Smart View app implements both a sender and a receiver. [S11] The published feature matrix on that page covers 2014–2017 televisions and lists BLE discovery, web casting, Wake on Wireless LAN, multitasking, and TLS. Wake on Wireless LAN is marked supported on 2016 and 2017 televisions, not on 2015, with a note of a compatibility issue on 2015 sets. TLS for native sender apps is documented as introduced in Android 2.3.7 (22 December 2016) and iOS 2.3.8 (14 December 2016), and as not supported for the JavaScript sender. [S11] [S12] The separate "Supported TVs" page lists 2014–2017 model floors and says supported models may vary by region. [S13]

The currently documented mobile packages are later than those TLS-introduction builds. The download page lists **Android Package 2.5.34, dated 25 July 2024**, and **iOS 14.1 Package 3.1.1, dated 5 April 2023**. The release notes for those versions are maintenance and toolchain updates — Android 2.5.34 says "Remove log sensitive when release," and iOS 3.1.1 says "Release SmartViewSDK Xcode14.1" — not a new remote-control protocol. [S12] The published television compatibility matrix remains centered on 2014–2017 sets. [S11] [S13] Later package dates must not be read as "the SDK binaries are all from 2016," and they also must not be read as a 2024 specification of consumer remote control.

That SDK can discover a television and launch an application that the SDK knows about. It does not, in the retrieved pages, document D-pad emulation of the television's own UI, a pairing token for `samsung.remote.control`, or ports 8001/8002. Treating Smart View as the V1 remote protocol would also imply a TV-side receiver application. That is a different setup from the television's own connection-approval prompt, and it is not what `docs/PRODUCT.md` describes. Later mobile package dates do not establish that a current Tizen television answers this sender/receiver architecture. The published matrix stops at 2017 even though Samsung's model-group table continues through 2026. [S14] Confidence: high that the published SDK is not a current public specification for AppT's desired consumer remote-control channel. Hardware validation: required before anyone treats the SDK as a viable control path on a 2020s television.
...
Reliable Samsung control is limited first by an unofficial, firmware-sensitive protocol, not by React Native. Native Android and native iOS do not get a vendor SDK that the retrieved public documents show as a current full remote. They would implement the same community protocol, wrap the published Smart View sender/receiver SDK, or depend on SmartThings cloud. The Smart View mobile packages have updates after 2016, but the published architecture and TV matrix do not become a consumer remote specification because of those dates. [S11] [S12] Those choices have the same protocol risk on every variant.
...
| Expo SDK 57 minimum iOS | Documented stack constraint, not a product decision. The SDK 57.0.0 compatibility row lists iOS 16.4+ and Xcode 26.4+. [S60] | The documented minimum for an Expo SDK 57 iPhone app is 16.4. Whether a lower deployment target still builds was not tested. A fully native app or a different stack could potentially use a different minimum, subject to Apple's then-current store rules. Whether AppT accepts 16.4+ must be considered before architecture selection. This report does not decide it. |
...
| `ACCESS_LOCAL_NETWORK` | Dangerous runtime permission added in API 37. Enforced for apps targeting SDK 37 or higher. Local TCP and UDP, including SSDP, require it. Android 16 used an opt-in and a temporary `NEARBY_WIFI_DEVICES` mapping. [S38] | Not in force for a documented Expo SDK 57 app. That SDK lists `targetSdkVersion` 36. [S60] Play's 31 August 2026 rule is also target API 36, not 37. [S39] Plan the prompt before any later raise to 37. The system device picker is not a fit for automatic multi-TV discovery. |
...
| Play target SDK / Expo SDK 57 target | New apps and updates must target API 36 as of 31 August 2026. [S39] Expo's SDK reference lists SDK 57.0.0 as Android 7+, `compileSdkVersion` 36, and `targetSdkVersion` 36. [S60] | The previously open question of SDK 57's documented target is closed: it is 36, which meets the current Play rule and does not by itself trigger Android 17's `ACCESS_LOCAL_NETWORK` enforcement. `expo-build-properties` can still raise it. [S56] Raising it to 37 would make that permission mandatory. [S38] |
...
Full native development does not gain a Samsung SDK the public documents show as a current remote. It pays for two UI codebases to reach the same protocol uncertainty and the same iOS entitlement and review rules. It could, however, choose a different minimum iOS version than Expo SDK 57's documented 16.4+. [S60] That is an OS-envelope difference, not a Samsung-control capability difference, and this report does not decide which minimum AppT accepts.
...
| Documented minimum OS | Expo SDK 57 documents Android 7+ and iOS 16.4+. [S60] This is a stack constraint, not a Samsung-control capability, and this report does not decide whether AppT accepts it. | Same documented SDK 57 envelope. `expo-build-properties` can override the iOS deployment target. [S56] Whether a lower target still builds and is accepted by the store was not tested. | Not established, in sources retrieved here, as a different envelope from the React Native version Expo SDK 57 ships. A different template could potentially set a different minimum. | Not bound to the Expo compatibility table. A different minimum-OS envelope is possible, subject to then-current store rules. Not decided here. |
...
- Android 17 documents `ACCESS_LOCAL_NETWORK` as a runtime permission for local TCP/UDP, including SSDP, for apps targeting API 37 or higher. Google Play's requirement as of 31 August 2026 is target API 36. Expo SDK 57 documents `compileSdkVersion` 36 and `targetSdkVersion` 36, so that documented target does not yet fall under the API 37 local-network permission. [S38] [S39] [S60]
...
- Samsung's published Smart View SDK is a sender/receiver multiscreen SDK. Its published television compatibility matrix remains centered on 2014–2017 sets. TLS for native senders was introduced in Android 2.3.7 (22 December 2016) and iOS 2.3.8 (14 December 2016). The current download page also lists later mobile packages: Android 2.5.34 (25 July 2024) and iOS 14.1 Package 3.1.1 (5 April 2023). Those later packages are not a basis for calling every documented binary a 2016 build, and they do not establish Smart View as a current public specification for AppT's consumer remote-control channel. [S11] [S12] [S13]
- Expo SDK 57.0.0 documents Android 7+, `compileSdkVersion` 36, `targetSdkVersion` 36, and iOS 16.4+ (Xcode 26.4+). [S60] The iOS 16.4+ minimum is a factual stack constraint that must be considered before architecture selection. This report does not decide whether AppT accepts it. A fully native or different stack could potentially have a different minimum-OS envelope.
...
- Expo SDK 57's documented minimums are Android 7+ and iOS 16.4+, with `targetSdkVersion` 36. [S60] A fully native or different stack could potentially have a different minimum-OS envelope. That is a consideration for architecture selection. It is not evidence that native code controls Samsung televisions more reliably, and this report does not choose a minimum.
...
| S12 | Smart View SDK download / release notes | https://developer.samsung.com/smarttv/develop/extension-libraries/smart-view-sdk/download.html | Re-retrieved 2026-09-22. Current listed mobile packages: Android Package 2.5.34 (25 July 2024, "Remove log sensitive when release"); iOS 14.1 Package 3.1.1 (5 April 2023, "Release SmartViewSDK Xcode14.1"). TLS introduction remains Android 2.3.7 (22 December 2016) and iOS 2.3.8 (14 December 2016). | The download page lists mobile package updates after 2016. The current iOS row is labeled an iOS 14.1 package. The page does not republish a post-2017 television compatibility matrix, and the later notes do not specify a consumer remote-control channel. | High for the package dates, labels, and note text; low that those dates prove 2020s TV compatibility | Yes, if anyone proposes this SDK as the control path |
...
| S56 | Expo build properties | https://docs.expo.dev/versions/latest/sdk/build-properties/ | Current SDK docs index; page body not fully quoted | Config-plugin mechanism that can override Android `targetSdkVersion` and the iOS deployment target. It is an override path, not the source of SDK 57's documented defaults. Those defaults are S60. | Medium for the override mechanism | No |
...
| S60 | Expo SDK reference, support for Android and iOS versions | https://docs.expo.dev/versions/latest/ | Re-retrieved 2026-09-22. Reference version labeled SDK 57 (latest). Compatibility row for 57.0.0. | Official table: SDK 57.0.0 supports Android 7+, `compileSdkVersion` 36, `targetSdkVersion` 36, iOS 16.4+, Xcode 26.4+. Same page's React Native table lists SDK 57.0.0 on React Native 0.86 and React 19.2.3. | High | No |
...
- **Smart View supported years versus model groups.** S13's television list stops in 2017. S14 continues through 2026. S12 lists later mobile package dates (Android 2.5.34 on 25 July 2024; iOS 14.1 Package 3.1.1 on 5 April 2023), but those notes do not extend the television matrix. The matrix, not the download-page dates, is the stale surface. Neither may be read as AppT's supported television range.
