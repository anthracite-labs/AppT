...
| A frozen purchase binding orphaned by an interrupted deletion | Freezing writes a random deletion-scoped `deletionId` on the binding and on the account record in one transaction, so the reconciliation has a defined lookup key. A frozen binding that names no `deleting` account is never released: it raises an operator alert, and an audited support action is the only other path out. | Enforced by AppT |
| A purchase released while the deleted identity can still authenticate | An `accountDeleted` release requires `authRemovalConfirmedAt`, written only from a successful `deleteUser` call or an Auth read that returns the user as gone. The job that finishes abandoned deletions refuses to release while the user still exists and leaves the binding frozen, so the purchase is unusable and unbindable rather than free. | Enforced by AppT |
...
| Debug or internal build pointing at production infrastructure | Per-variant configuration with CI checks that the debug variant cannot resolve production identifiers, that the release variant cannot resolve development identifiers, that the internal and production release artifacts from one commit differ only in environment configuration once signature material is stripped, and that each environment's App Check registration names only its own signing certificate. | Enforced by AppT |
...
| Compromised release artifact | Play App Signing, with the app-signing key held by Google; the upload key is a CI secret; the outside-Play tester build is signed with a separate internal signing key whose certificate is registered only in `appt-internal`; mapping files are uploaded from the release workflow. | Best-effort |
| Lost, leaked, or misused internal signing key | That key signs tester builds only. Its certificate is accepted by `appt-internal` and by no production project, so a build it signs cannot authenticate to `appt-prod`. The recovery path is re-enrolling testers and re-registering the certificate; production installs and the production registration are untouched. | Best-effort |
...
Controls above become tests in [testing.md](testing.md). At minimum: `identityMismatchDoesNotSendToken`, `noPlaintextTokenAfterTlsPairing`, `malformedFrameDoesNotEscapeSession`, `backendRejectsClientSuppliedUid`, `clientFirestoreAccessDenied`, `trialMarkerKeyRotationResolvesOldMarkers`, `testPurchaseDoesNotGrantLifetime`, `proofSignatureTamperDenied`, `clockRollbackDoesNotExtendEntitlement`, `revocationAppliesOnNextEntry`, `deletionFreezesBeforeAuthDelete`, `deletionRetryConverges`, `frozenBindingRetainsOwnerCorrelation`, `reconciliationRequiresAuthRemoval`, `releaseRequiresAuthRemovalProof`, `trialAttachIsIdempotent`, `provisionalUsesLocalKey`, `redactedReportContainsNoFixtureSecret`, `noTelemetryDependency`, `noDiagnosticsUploadPath`, `samsungGraphExcludesFirebase`, `releaseVariantCannotReachDevelopment`, `signingIdentityMatchesChannel`, `appCheckRegistrationMatchesChannel`.
