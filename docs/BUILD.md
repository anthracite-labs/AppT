...
## S01 verification status

The Android half of the S01 verification set (`assembleDebug`, `lintDebug`,
`testDebugUnitTest`, `dependencyLockCheck`, the guards, the dependency-insight
checks, `:macrobenchmark:assembleBenchmark`, and the device install) has **not**
been executed locally. The implementing sandbox had no JDK and no Android SDK,
and no egress to `dl.google.com`, `repo1.maven.org` or `services.gradle.org`, so
Gradle could not resolve or run at all. CI is the first environment that
executes them, which is also why the committed lockfiles and the
`verification-metadata.xml` checksum entries are generated by the bootstrap
path in `.github/workflows/ci.yml` rather than committed from a local run.

Executed and passing locally: the backend package
(`npm ci/typecheck/lint/format:check/test --prefix backend`) and the secret
scanner (self-test plus a full tree scan).
