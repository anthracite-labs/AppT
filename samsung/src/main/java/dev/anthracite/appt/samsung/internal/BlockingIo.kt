 ...
 * or an SSDP receive would keep a cancelled scan alive until its socket timeout, pushing
 * `Finished` past the bound and holding the multicast lock longer than the scan. Closing the
 * socket makes the blocked call fail immediately, and that failure is reported as the cancellation
 * it is rather than as a network error.
 */
