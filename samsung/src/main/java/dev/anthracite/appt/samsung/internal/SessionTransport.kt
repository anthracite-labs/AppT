 ...
 *
 * This is a test detail of the `samsung` module, not a caller-facing interface: `app` never sees it,
 * and the wire format, ports, key strings, URL and TLS mechanics stay behind it
...
internal interface SessionTransport {
     ...
     *
     * Returns null when the socket cannot be opened at all; the caller then reports the television as
     * unreachable. Containment is the adapter's job: nothing about a socket, TLS or parser failure
     * crosses the seam as a thrown exception.
     */
...
internal interface SessionConnection {
    /**
     * Inbound frames, in arrival order, each already bounded by the adapter. The flow completes when
     * the socket ends. Malformed content is contained by the parser, not by the caller.
     */
    val frames: Flow<String>

    /**
     * The SHA-256 of the SubjectPublicKeyInfo the television presented, or null on the plaintext
     * channel. On first contact this is the candidate pin: it stays in memory for the session and is
     * discarded on close. Persisting it with the token is S04.
     */
    val certificateIdentity: String?

    /**
     * Writes one already-encoded frame. Returns false when the frame could not be written, including
     * when the unsent-frame cap is already reached, so the caller reports
