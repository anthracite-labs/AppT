 ...
 *
 * Remembering a television is one Room statement: the user selects a controllable card, this
 * upserts the profile, and only then does `ActiveRemoteHost` open the session. Nothing here reads a
 * network source, and nothing here can hold a pairing secret.
 *
 * @property now device millis, injected so tests are not on the wall clock.
 */
class TvProfiles(
    private val dao: TvProfileDao,
     ...
     * name never overwrites one the user typed. An `Unsupported` television never reaches here, so
     * no row is created for one.
     *
